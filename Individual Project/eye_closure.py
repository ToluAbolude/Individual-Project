...
def detectEar():
	...
	while True:
		...
		for rect in rects:
			...
			lipsBottom = mouth[9]
			#lefteyebrow = leftEyebrow[3]
			nosePoint = nose[6]
			
			A = dist.euclidean(righteyebrow,jawArea[0])
			B = dist.euclidean(lipsBottom,jawArea[1])
			#C = dist.euclidean(lefteyebrow,jawArea[2])
			
			faceSize = A + (B-1)
			
			ear = (100*(leftEAR + rightEAR) / 2.0)/faceSize
			if ear < EYE_AR_THRESH:
				...
				if COUNTER >= EYE_AR_CONSEC_FRAMES:
					...
					if not ALARM_ON:
						...
						t = Thread(target=version2.sound_alarm,args=("alarm.wav",))
						t.deamon = True
						t.start()

					# draw an alarm on the frame
					cv2.putText(frame, "EYES CLOSED ALERT!", (0, 30),#changed
		...
		# show the frame
		cv2.imshow("Driver Eye Closure Detection", frame)
